...
public class PropertyCheckerLocal implements Runnable {
	private final int start;
	private final int end;
	List<Integer> numbers = new ArrayList<>();
	List<String> stringRep = new ArrayList<>();

	public PropertyCheckerLocal(int start, int end) {
		this.start = start;
		this.end = end;
	}
...
//        calls function to check all numbers between 1 and 1000
	 ...
	 */
	public void checkForANumber(int num) {
...
//        System.out.println(fullString);
		this.numbers.add(numbers.size(), originalnum);
		this.stringRep.add(stringRep.size(), fullString);
	}
